...
# --- ВЕРСИЯ 26.0 (FIXED RANGES LOGIC & MULTI-SELECT) ---
...
<style>
    ...
    /* СТОЛ */
    .game-area { position: relative; width: 100%; max-width: 500px; height: 320px; margin: 0 auto 10px auto; background: radial-gradient(ellipse at center, #2e7d32 0%, #1b5e20 100%); border: 10px solid #3e2723; border-radius: 160px; box-shadow: 0 5px 20px rgba(0,0,0,0.6); }
...
with tab_trainer:
    ...
    with st.expander("⚙️ Settings", expanded=False):
        if not ranges_db:
            st.error("Ranges.json not found or invalid format.")
            st.stop()
            ...
            for sc in selected_scenarios:
                if sc in ranges_db[src]:
                    ...
                    for sp in spots:
                        ...
                        elif train_mode == "Early (EP/MP)": is_match = any(x in u for x in ["EP", "UTG", "MP"])
                        elif train_mode == "Late (CO/BU/SB)": is_match = any(x in u for x in ["CO", "BU", "BTN", "SB"])
