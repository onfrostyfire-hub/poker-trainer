...
def show():
    # --- МОБИЛЬНЫЙ CSS (БЕЗ КОМПРОМИССОВ) ---
    st.markdown("""
    <style>
        /* === ГЛОБАЛЬНЫЙ ФИКС ДЛЯ МОБИЛОК === */
        
        /* 1. Отступ сверху (чтобы не лезло под камеру) */
        .block-container { 
        ...
        /* 2. ПРИНУДИТЕЛЬНО В РЯД (Запрещаем перенос колонок) */
        ...
        /* 3. СТИЛИ КНОПОК ДЕЙСТВИЙ (БОЛЬШИЕ) */
        div.stButton > button { 
            width: 100%; 
        ...
        /* ЭЛЕМЕНТЫ СТОЛА */
        .seat { position: absolute; width: 40px; height: 40px; background: #222; border: 1px solid #444; border-radius: 6px; display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 5; }
        ...
        .dealer-mob { position: absolute; width: 14px; height: 14px; background: #ffc107; border-radius: 50%; color: #000; font-weight: bold; font-size: 8px; display: flex; justify-content: center; align-items: center; z-index: 6; }
        ...
        /* HERO */
        .hero-mob { position: absolute; bottom: -25px; left: 50%; transform: translateX(-50%); display: flex; gap: 4px; z-index: 10; background: #222; padding: 3px 6px; border-radius: 8px; border: 1px solid #ffc107; }
        ...
        .tl-mob { position: absolute; top: 1px; left: 2px; font-weight: bold; font-size: 14px; line-height: 1; }
    ...
    for i in range(1, 6):
        ...
        chips = ""
        if pos == "BTN": chips = '<div class="dealer-mob" style="top:-8px; right:-8px;">D</div>'
    ...
    if not st.session_state.srs_mode:
        ...
        with c1:
            if st.button("FOLD"):
                ...
                st.session_state.msg = "✅ Correct" if corr else f"❌ Err (Raise {int(ans_w*100)}%)"
        ...
        # Переопределяем стили для SRS кнопок (они тоже должны быть в ряд)
        st.markdown("""
        <style>
            div[data-testid="column"] div.stButton > button { height: 60px !important; background: #343a40 !important; font-size: 14px !important; box-shadow: none !important; border: 1px solid #555; }
        </style>
        """, unsafe_allow_html=True)
