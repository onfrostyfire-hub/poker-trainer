...
def show():
    ...
    <style>
        ...
        /* ФИШКИ */
        .dealer-button { position: absolute; width: 22px; height: 22px; background: #ffc107; border: 1px solid #e0a800; border-radius: 50%; color: #000; font-weight: bold; font-size: 11px; display: flex; justify-content: center; align-items: center; box-shadow: 1px 1px 3px rgba(0,0,0,0.5); z-index: 15; }
        .poker-chip { width: 20px; height: 20px; background: #222; border: 3px dashed #d32f2f; border-radius: 50%; box-shadow: 1px 1px 2px rgba(0,0,0,0.7); }
        .chip-3bet-desk { width: 22px; height: 22px; background: #d32f2f; border: 2px solid #fff; border-radius: 50%; box-shadow: 0 2px 5px rgba(0,0,0,0.5); }
        .blind-stack { position: absolute; z-index: 15; display: flex; flex-direction: column; align-items: center; }
        
        .pos-1 { bottom: 20%; left: 10%; } .pos-2 { top: 20%; left: 10%; } .pos-3 { top: -30px; left: 50%; transform: translateX(-50%); } .pos-4 { top: 20%; right: 10%; } .pos-5 { bottom: 20%; right: 10%; }
        
        /* HERO & RNG */
        .hero-panel { position: absolute; bottom: -45px; left: 50%; transform: translateX(-50%); background: #212529; border: 2px solid #ffc107; border-radius: 12px; padding: 6px 18px; display: flex; gap: 8px; box-shadow: 0 0 20px rgba(255, 193, 7, 0.2); z-index: 10; align-items: center; }
        ...
        .raise-btn button { background: #d63384 !important; color: white !important; }
        .raise-btn button:hover { background: #c2185b !important; }
        
        .open-raise-btn button { background: #2e7d32 !important; color: white !important; }
    ...
    with col_center:
        ...
        if is_defense_mode:
            st.markdown('<div style="text-align:center;color:#888;font-size:12px;margin-bottom:10px;">📉 0..Freq → Action | 📈 Freq..100 → Fold</div>', unsafe_allow_html=True)
    ...
    with col_right:
        if st.session_state.srs_mode:
            ...
            # Мы передаем ВЕСЬ словарь data, чтобы utils сам решил, какие цвета рисовать
